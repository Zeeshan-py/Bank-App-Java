...
public class BankGUI extends JFrame {
    ...
    private String currentUserRole; // "manager" or "client"
    private String currentUserId; // Client ID for clients, "admin" for manager
    ...
    public BankGUI() {
        ...
        // Note: Ideally, we should not call setVisible(true) until after login if we
        // want to hide the main frame
        // But the original code structure handles this by exiting if login fails.
        // We will stick to the logic but ensure the frame is ready.
        if (showLoginDialog()) {
            createMainPanel();
            setVisible(true);
        } else {
    ...
    private boolean showLoginDialog() {
        JDialog loginDialog = new JDialog(this, "Login - National Bank", true);
        // loginDialog.setSize(500, 550); // Removed fixed size in favor of pack()
        ...
        String[] roles = { "Manager", "Client" };
        JComboBox<String> roleCombo = new JComboBox<>(roles);
        roleCombo.setFont(new Font("Segoe UI", Font.PLAIN, 15));
        roleCombo.setBackground(Color.WHITE);
        // Removed setPreferredSize for responsiveness

        JLabel userLabel = new JLabel("Username / Client ID:");
        ...
        // Button Panel
        JPanel buttonPanel = new JPanel(new GridLayout(1, 2, 10, 0)); // GridLayout for equal sizing
        buttonPanel.setBackground(PANEL_COLOR);

        JButton loginBtn = createStyledButton("Login", SUCCESS_COLOR);
        JButton cancelBtn = createStyledButton("Exit", DANGER_COLOR);
        ...
        ActionListener loginAction = e -> {
            ...
            if (role.equals("manager")) {
                ...
                // Client login - check if client exists
                Client client = null;
                ...
                if (client != null && password.equals("client123")) {
                    currentUserRole = "client";
                    currentUserId = client.getId();
                    currentUserName = client.getPersonalInfo().getName();
                    loginSuccess[0] = true;
                    loginDialog.dispose();
                } else {
                    JOptionPane.showMessageDialog(loginDialog,
                            "Invalid client credentials or client not found",
                            "Login Failed", JOptionPane.ERROR_MESSAGE);
                }
            }
        };
        ...
        loginDialog.pack(); // Adapt to content
        loginDialog.setMinimumSize(new Dimension(500, 550)); // Prevent it from being too small
        loginDialog.setLocationRelativeTo(null); // Center after packing
    ...
    private void createMainPanel() {
        ...
        if (currentUserRole.equals("manager")) {
            ...
            // Client sees only their accounts and transactions
            tabbedPane.addTab("   My Accounts  ", createClientAccountsPanel());
            tabbedPane.addTab("   Transactions  ", createTransactionsPanel());
        }
    ...
    private JPanel createHeaderPanel() {
        ...
        // headerPanel.setPreferredSize(new Dimension(0, 80)); // Let it auto-size based
        // on content padding
        ...
        JLabel userInfoLabel = new JLabel(String.format("%s: %s",
        ...
        logoutBtn.setMaximumSize(new Dimension(80, 25));
    ...
    private JPanel createDashboardPanel() {
        ...
        // statsPanel.setPreferredSize(new Dimension(0, 160)); // Let contents decide
        // height
    ...
    private JButton createActionButton(String text, Color color) {
        ...
        button.setCursor(new Cursor(Cursor.HAND_CURSOR));
        // button.setPreferredSize(new Dimension(200, 60)); // Let layout manager handle
        // size
    ...
    private JPanel createTransactionPanel(String title, Color color, boolean isDeposit) {
        ...
        // Form panel - Using GridBagLayout instead of BoxLayout for better resizing
        JPanel formPanel = new JPanel(new GridBagLayout());
        formPanel.setBackground(PANEL_COLOR);
        ...
        // Removed setMaximumSize which was causing issue "text bar not show"
        ...
        gbc.gridy = 2;
        gbc.insets = new Insets(20, 0, 5, 0); // Extra space before next label
    ...
    private JButton createStyledButton(String text, Color color) {
        ...
        button.setBorderPainted(false);
        button.setCursor(new Cursor(Cursor.HAND_CURSOR));
        // button.setPreferredSize(new Dimension(150, 42)); // Let layout manager handle
        // size
    ...
    private void showAddClientDialog() {
        if (!currentUserRole.equals("manager")) {
            showMessage("Only managers can add clients!", "Access Denied", JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    private void showAddAccountDialog() {
        if (!currentUserRole.equals("manager")) {
            showMessage("Only managers can create accounts!", "Access Denied", JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    private void removeClient() {
        if (!currentUserRole.equals("manager")) {
            showMessage("Only managers can remove clients!", "Access Denied", JOptionPane.ERROR_MESSAGE);
            return;
        }
